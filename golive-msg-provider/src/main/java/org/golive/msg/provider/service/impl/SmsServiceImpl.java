package org.golive.msg.provider.service.impl;
...
import org.golive.framework.redis.starter.key.MsgProviderCacheKeyBuilder;
import org.golive.msg.dto.MsgCheckDTO;
import org.golive.msg.enums.MsgSendResultEnum;
import org.golive.msg.provider.config.ApplicationProperties;
import org.golive.msg.provider.config.SmsTemplateIDEnum;
import org.golive.msg.provider.config.ThreadPoolManager;
import org.golive.msg.provider.dao.mapper.SmsMapper;
import org.golive.msg.provider.dao.po.SmsPO;
...
public class SmsServiceImpl implements ISmsService {
    ...
    public MsgSendResultEnum sendLoginCode(String phone) {
        ...
        // 1.生成验证码,并缓存到redis; 4位, 6位(√), 有效期(30s, 60s), 同一个手机号不能重发
        String codeCacheKey = msgProviderCacheKeyBuilder.buildSmsLoginCodeKey(phone);
        if(redisTemplate.hasKey(codeCacheKey)){  // 判断是否重发,若redis中已缓存,则为已发送,返回发送过于频繁提醒
    ...
    private boolean sendSmsToCCP(String phone, Integer code) {
        ...
        try {
            ...
            String templateId= SmsTemplateIDEnum.SMS_LOGIN_CODE_TEMPLATE.getTemplatedId();
    ...
    //     //主账号,登陆云通讯网站后,可在控制台首页看到开发者主账号ACCOUNT SID和主账号令牌AUTH TOKEN
    //     String accountSId = "2c94811c8cd4da0a018eccffc54e597f";
    //     String accountToken = "a3e8f622bc2c45d98aa2d26bda59f5f5";
    //     //请使用管理控制台中已创建应用的APPID
    //     String appId = "2c94811c8cd4da0a018eccffc6d95986";
    //     CCPRestSmsSDK sdk = new CCPRestSmsSDK();
    //     sdk.init(serverIp, serverPort);
    //     sdk.setAccount(accountSId, accountToken);
    //     sdk.setAppId(appId);
    //     sdk.setBodyType(BodyType.Type_JSON);
    //     String to = "15960212183";  // 仅可以测试官网中填入的测试号码
    //     String templateId= "1";
    //     // 测试开发支持的文案如下:【云通讯】您的验证码是{1}，请于{2}分钟内正确输入。其中{1}和{2}为短信模板参数。
    //     // 随机一个验证码验证码
    //     int code = RandomUtils.nextInt(1000, 9999);
